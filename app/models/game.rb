...
class Game < ActiveRecord::Base
  ...
  def self.get_previous_week_results
  ...
  def self.determine_spread_winners
    previous_week = get_current_week - 1
  ...
  def self.get_games
    ...
    find_all("tbody tr td table tr td table tbody tr td span table tbody tr td").each do |info|
...
def get_results(week)
  ...
  find_all(".away .away div h2").each do |away_team|
    away_teams << teams_hash[away_team.text]
  end

  find_all(".home .home div h2").each do |home_team|
    home_teams << teams_hash[home_team.text]
  end

  find_all(".away .total span").each do |away_score|
    away_scores << away_score.text.to_i
  end

  find_all(".home .total span").each do |home_score|
